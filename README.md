...
## 기능
- 다운로드 목록: 진행바·속도·ETA, 필터(전체/진행중/완료/실패/일시정지), 카드 **좌 스와이프 삭제 / 우 스와이프 일시정지·재개**, 탭하면 파일 목록·경로 복사·DSM 열기
- 장바구니(JD 링크그래버): 추가한 링크가 먼저 담기는 곳. 살아 있는지 확인한 뒤 패키지별/전체 시작, 오프라인 제거
- 추가: 붙여넣기(잡문 섞여도 URL만 추출), 저장 위치(기본 다운로드 폴더가 기본값, 하위 폴더 추가 가능), **장바구니에 담기**(검증 결과 확인 후 선택 시작) / **즉시 다운로드**(장바구니 생략, 검증 후 자동 시작) 두 버튼
...
| `BROWSER_URL` | (없음) | 폰용 쿠키 갱신 크롬([browser/](browser/README.md))의 **기본** 주소. ⚙ 설정의 '쿠키 갱신 접속' 버튼. 사용자는 같은 설정에서 자기 주소로 덮어쓸 수 있음(브라우저에 저장). 둘 다 없으면 숨김 |
