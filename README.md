...
- 계정 탭: 호스터 프리미엄 계정 목록·추가·변경·사용 여부·갱신·삭제 (`accountsV2`). **쿠키 로그인 호스터(terabox 등)는 비밀번호 대신 브라우저 쿠키 내보내기 텍스트를 붙여넣는 모드** 제공(도메인 입력 시 자동 전환). 비밀번호/쿠키는 JD로 전달만, 앱에 저장 안 함
